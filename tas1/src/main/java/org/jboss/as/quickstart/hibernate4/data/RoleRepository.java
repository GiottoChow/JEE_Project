...
public class RoleRepository {
	...
	public List<Role_bkup> findAllOrderedByName() {
		// using Hibernate Session and Criteria Query via Hibernate Native API
		Session session = (Session) em.getDelegate();
		Criteria cb = session.createCriteria(Role_bkup.class);
		// cb.addOrder(Order.asc("name"));
		return cb.list();
		// return members;
	}

	public Role_bkup findById(Long id) {
		return em.find(Role_bkup.class, id);
	}
}
